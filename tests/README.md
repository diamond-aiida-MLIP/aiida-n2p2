...
- **v0.4.0:** ``auto_restart=True`` loops until target ``epochs`` are reached,
  even when a CalcJob exits early (e.g. walltime). Requires parseable partial
  outputs (``learning-curve.out`` + last weights). Scale/train parsers poll
  remote folders when retrieve is premature (OAR on Dahu).
- **Manual restart:** two ``N2p2TrainWorkChain`` submissions; segment~2 uses
  ``additional_epochs`` as **remaining** epochs in ``input.nn``.
